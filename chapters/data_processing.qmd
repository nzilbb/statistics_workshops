...
cat(
  "In the output of `read_csv()` above, F1_50 is listed as a ", 
  mcq(opts),
  "column"
)
```
...
If we want to filter a data frame, we use `filter()`. The first argument to
`filter()` is a data frame. This argument is usually filled by the use of a pipe
function (i.e., the `filter()` function usually appears within a pipe). The
remaining arguments are a series of logical statements which say which rows you
want to keep and which you want to remove. By 'logical' recall that we mean 
`TRUE` or `FALSE`. Rows which produce `TRUE` will be kept, those which don't 
will not.
...
Consider the following code. Assume that `vowels` has columns called
`F1_50` and` F2_50`.
...
- After running the code, `qb_vowels` will have a column called 
`first_formant`: `r torf(TRUE)`.
- After running the code, `qb_vowels` will have a column called 
`F2 _50`: `r torf(FALSE)`.
...
- There are 11 vowels and 2 genders in `vowels`. How many rows are there in
`vowel_summary`? `r fitb(22)`

```{r}
#| output: asis
#| echo: false
opts <- c(
  answer = "To return the number of rows in each group.",
  "To return the number of rows in the entire data frame.",
  "To ensure the script is reproducible."
)
...
- Experiment with the `mean()` functiona. What does the function output if it 
gets `NA` values (e.g. `mean(c(1, 2, 3, NA, 5))`)? `r mcq(c(answer = "NA", "7.25"))`
